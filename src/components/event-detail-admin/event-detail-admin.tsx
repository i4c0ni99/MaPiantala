...
export const EventDetailAdminCard: React.FC<ICardEventDetailAdmin> = function ({
    event,
    onSubmission,
}) {
    const [partecipantLimit] = useState<number[]>([
        50, 100, 200, 300, 400, 500, 600,
    ]);

    const handleChange = (e: ChangeEvent<HTMLSelectElement>) => {
        if (event) event.partecipantsNumer = parseInt(e.target.value);
    };
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        if (event) {
            event.isPublic = true;
            if (onSubmission) onSubmission(event);
        }
    };
                            ...
                            <label className="form-control w-full max-w-xs">
                                <div className="label w-64">
                                    <span className=" font-bold">
                                        il limite di patrecipanti è{" "}
                                        {event.partecipantsNumer}
                                    </span>
                                </div>
                                <select
                                    name="select"
                                    value={event.partecipantsNumer}
